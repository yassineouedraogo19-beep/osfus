    ...
    // Toutes les pharmacies avec coordonnées
    const pharmacies = [
      {name:"Pharmacie de la Paix", coords:[12.378422771475776, -1.5233600170264758], phone:"+226 25 30 12 34", address:""},
      {name:"Pharmacie Sainte Véronique", coords:[12.32783350245309, -1.6325353891826342], phone:"+226 63 26 04 04", address:"Zagtouli"},
    ...
    // Déterminer le groupe de garde selon la semaine
   function getGroupeDeGarde(date = new Date()) {
  const reference = new Date("2025-12-20"); // début du groupe III
...
// Icône utilisateur : pointeur goutte bleu
...
navigator.geolocation.getCurrentPosition(pos => {
  ...
  map.setView([userLat, userLng], 14); // centrer sur l’utilisateur
});
  ...
  pharmacies.forEach((ph, index) => {
    ...
    marker.bindPopup(`
      ...
      📍 ${ph.address}<br>
      📞 ${ph.phone}<br>
      ${isGarde ? "✅ De garde (Groupe " + groupe + ")" : "❌ Non de garde"}
    `);

 // 👉 Clic sur le marker = afficher itinéraire
  marker.on("click", () => showRoute(index));
    ...
    div.innerHTML = `
      ...
      <p>📍 ${ph.address}</p>
      <p>📞 ${ph.phone}</p>
      <p class="${isGarde ? "garde" : "nongarde"}">${isGarde ? "✅ De garde" : "Non de garde"}</p>
      <button onclick="showRoute(${index})">Itinéraire</button>
    `;
...
function showRoute(index, { refresh } = { refresh: false }) {
  if (!window.userLocation) {
    alert("Position utilisateur non trouvée.");
    return;
  }
  const ph = pharmacies[index];
  if (!ph || !isValidCoords(ph.coords)) {
    console.error('Coordonnées pharmacie invalides:', ph?.coords);
    alert("Coordonnées de la pharmacie invalides.");
    return;
  }
